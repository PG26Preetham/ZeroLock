...
void UZero_BaseReloadAbility::OnFinish()
{
	...
	if (AbilitySystemComp && ReloadEffect)
	{
		...
		if (SpecHandle.IsValid())
		{
			...
			FGameplayEffectQuery ReloadEffectQuery;
			ReloadEffectQuery.EffectDefinition = ReloadingEffect;
			
			AbilitySystemComp->RemoveActiveEffects(ReloadEffectQuery);
                                              ...
                                              const FGameplayEventData* TriggerEventData)
{
	AZeroLockCharacter* Hero = Cast<AZeroLockCharacter>(GetAvatarActorFromActorInfo());
	UAbilitySystemComponent* AbilitySystemComp = Hero->GetAbilitySystemComponent();
	if (AbilitySystemComp && ReloadingEffect)
	{
		FGameplayEffectContextHandle EffectContext =AbilitySystemComp->MakeEffectContext();
		EffectContext.AddSourceObject(this);


		FGameplayEffectSpecHandle SpecHandle = AbilitySystemComp->MakeOutgoingSpec(ReloadingEffect, 1, EffectContext);
