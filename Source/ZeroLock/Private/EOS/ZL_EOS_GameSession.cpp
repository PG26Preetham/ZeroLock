...
#include "EOS/ZL_EOS_GameSession.h"
...
void AZL_EOS_GameSession::HandleCreateSessionCompleted(FName EOSSessionName, bool bWasSuccessful)
{
...
bool AZL_EOS_GameSession::ProcessAutoLogin()
{
    return true;
}

void AZL_EOS_GameSession::RegisterPlayer(APlayerController* NewPlayer, const FUniqueNetIdRepl& UniqueId,
    bool bWasFromInvite)
{
    ...
    if (IsRunningDedicatedServer()) // Only run this on the dedicated server
    {
        ...
        if (!Session->RegisterPlayer(SessionNameX, *UniqueId, false))
        {
...
void AZL_EOS_GameSession::HandleRegisterPlayerCompleted(FName XSessionNameX, const TArray<FUniqueNetIdRef>& PlayerIds,
    bool bWasSuccesful)
{
...
void AZL_EOS_GameSession::UnregisterPlayer(const APlayerController* ExitingPlayer)
{
    ...
    if (IsRunningDedicatedServer())
    {
        ...
        if (ExitingPlayer->PlayerState) // If the player leaves ungracefully this could be null
        {
            ...
            if (!Session->UnregisterPlayer(SessionNameX, *ExitingPlayer->PlayerState->GetUniqueId()))
            {
...
void AZL_EOS_GameSession::HandleUnregisterPlayerCompleted(FName XSessionNameX, const TArray<FUniqueNetIdRef>& PlayerIds,
    bool bWasSuccesful)
{
...
void AZL_EOS_GameSession::StartSession()
{
    ...
    if (!Session->StartSession(SessionNameX))
    {
...
void AZL_EOS_GameSession::HandleStartSessionCompleted(FName XSessionNameX, bool bWasSuccessful)
{
...
void AZL_EOS_GameSession::NotifyLogout(const APlayerController* ExitingPlayer)
{
...
void AZL_EOS_GameSession::EndSession()
{
    ...
    if (!Session->EndSession(SessionNameX))
    {
...
void AZL_EOS_GameSession::HandleEndSessionCompleted(FName XSessionNameX, bool bWasSuccessful)
{
...
void AZL_EOS_GameSession::DestroySession()
{
    ...
    if (!Session->DestroySession(SessionNameX))
    {
...
void AZL_EOS_GameSession::HandleDestroySessionCompleted(FName XSessionNameX, bool bWasSuccessful)
{
...
void AZL_EOS_GameSession::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void AZL_EOS_GameSession::BeginPlay()
{
    Super::BeginPlay();
...
void AZL_EOS_GameSession::CreateSession(FName KeyName, FString KeyValue)
{
    ...
    if (!Session->CreateSession(0, SessionNameX, *SessionSettings))
    {
        UE_LOG(LogTemp, Warning, TEXT("Failed to create session!"));
		Session->ClearOnCreateSessionCompleteDelegate_Handle(CreateSessionDelegateHandle);
        CreateSessionDelegateHandle.Reset();
